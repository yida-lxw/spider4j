 ...
 * href="http://portal.acm.org/citation.cfm?id=6621">min-max heap</a> developed
 * by Atkinson, et al. Unlike many other double-ended priority queues, it stores
 * elements in a single array, as compact as the traditional heap data structure
 * used in {@link PriorityQueue}.
 * 
 * <p>
 ...
 * all the forms of {@link #poll} and {@link #remove()}) run in
 * {@code O(log n) time}
...
public final class MinMaxPriorityQueue<E> extends AbstractQueue<E> {
	 ...
	 * {@code MinMaxPriorityQueue} instances that use {@code comparator} to
	 * determine the least and greatest elements.
	 */
	...
	/**
	 * Creates and returns a new builder, configured to build
	 * {@code MinMaxPriorityQueue} instances sized appropriately to hold
	 * {@code expectedSize} elements.
	 */
	...
	/**
	 * Creates and returns a new builder, configured to build
	 * {@code MinMaxPriorityQueue} instances that are limited to
	 * {@code maximumSize} elements. Each time a queue grows beyond this bound,
	 * it immediately removes its greatest element (according to its
	 ...
	 * greatest element (according to its comparator), which may be
	 * {@code element} itself.
	 * 
	...
	MoveDesc<E> removeAt(int index) {
		...
		if (bubbledTo == vacated) {
			...
			return (bubbledTo < index) ? new MoveDesc<E>(toTrickle,
	...
	private class QueueIterator implements Iterator<E> {
		 ...
		 * {@code skipMe} and returns {@code size()} if there is no such
		 * element.
		 */
	...
	private int calculateNewCapacity() {
		...
		int newCapacity = (oldCapacity < 64) ? (oldCapacity + 1) * 2
				: (oldCapacity / 2) * 3;
		if (newCapacity < 0) {
			newCapacity = Integer.MAX_VALUE; // overflow - hotspot will throw
												// OOME
		}
