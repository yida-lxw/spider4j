...
import com.yida.spider4j.crawler.utils.collection.Table.Cell;
...
public final class Tables {
	 ...
	 */
	public static <R, C, V> Cell<R, C, V> immutableCell(@Nullable R rowKey,
			@Nullable C columnKey, @Nullable V value) {
		return new ImmutableCell<R, C, V>(rowKey, columnKey, value);
	}

	private static class ImmutableCell<R, C, V> extends AbstractCell<R, C, V>
	 ...
	 * generated table always returns the same value as calling
	 * {@code get(rowKey, columnKey)} on the original table. Updating the
	 * original table changes the contents of the transposed table and vice
	 * versa.
	 * 
	 * <p>
	 ...
	 * example, in a {@link HashBasedTable} instance,
	 * {@code rowKeySet().iterator()} supports {@code remove()} but
	 * {@code columnKeySet().iterator()} doesn't. With a transposed
	...
	private static class TransposeTable<C, R, V> implements Table<C, R, V> {
		...
		// eclipse doesn't like the raw type
		...
		class CellSet extends
				TransformedCollection<Cell<R, C, V>, Cell<C, R, V>> implements
				Set<Cell<C, R, V>> {
			...
			public boolean contains(Object obj) {
				if (obj instanceof Cell) {
					Cell<?, ?, ?> cell = (Cell<?, ?, ?>) obj;
					return original.cellSet().contains(
							immutableCell(cell.getColumnKey(),
			...
			public boolean remove(Object obj) {
				if (obj instanceof Cell) {
					Cell<?, ?, ?> cell = (Cell<?, ?, ?>) obj;
					return original.cellSet().remove(
							immutableCell(cell.getColumnKey(),
									cell.getRowKey(), cell.getValue()));
				}
				return false;
