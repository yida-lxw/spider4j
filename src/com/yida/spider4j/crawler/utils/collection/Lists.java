...
public final class Lists {
	 ...
	 * use an overload of {@link ImmutableList#of()} (for varargs) or
	...
	public static <E> ArrayList<E> newArrayList(Iterable<? extends E> elements) {
		...
		return (elements instanceof Collection) ? new ArrayList<E>(
	 ...
	 * 
	 * <p>
	 * <b>Note:</b> if you know the exact size your list will be, consider using
	 * a fixed-size list ({@link Arrays#asList(Object[])}) or an
	 * {@link ImmutableList} instead of a growable {@link ArrayList}.
	 * 
	 * <p>
	 * <b>Note:</b> If you have only an <i>estimate</i> of the eventual size of
	...
	public static <E> ArrayList<E> newArrayListWithCapacity(int initialArraySize) {
		Preconditions.checkArgument(initialArraySize >= 0); // for GWT.
	 ...
	 * 
	 * <p>
	 * <b>Note:</b> If you know the <i>exact</i> number of elements the list
	 ...
	 * {@code rest} array will be reflected in the returned list. Unlike
	...
	private static class OnePlusArrayList<E> extends AbstractList<E> implements
	...
	private static class TwoPlusArrayList<E> extends AbstractList<E> implements
			Serializable, RandomAccess {
		final E first;
		final E second;
		final E[] rest;
	 ...
	 * {@code fromList}. The returned list is a transformed view of
	 * {@code fromList}; changes to {@code fromList} will be reflected in the
	 ...
	 * list. The returned list always implements {@link Serializable}, but
	 * serialization will succeed only when {@code fromList} and
	 * {@code function} are serializable. The returned list is threadsafe if the
	 * supplied list and function are.
	 */
	public static <F, T> List<T> transform(List<F> fromList,
			Function<? super F, ? extends T> function) {
		return (fromList instanceof RandomAccess) ? new TransformingRandomAccessList<F, T>(
				fromList, function) : new TransformingSequentialList<F, T>(
				fromList, function);
	}
	 ...
	 * @see Lists#transform
	 */
	private static class TransformingSequentialList<F, T> extends
	...
	private static class TransformingRandomAccessList<F, T> extends
			AbstractList<T> implements RandomAccess, Serializable {
		final List<F> fromList;
		final Function<? super F, ? extends T> function;
	...
	public static <T> List<List<T>> partition(List<T> list, int size) {
		...
		return (list instanceof RandomAccess) ? new RandomAccessPartition<T>(
				list, size) : new Partition<T>(list, size);
	}
	...
	private static final class StringAsImmutableList extends
		...
		public int indexOf(Object object) {
			return (object instanceof Character) ? string
					.indexOf((Character) object) : -1;
		}

		@Override
		public int lastIndexOf(Object object) {
			return (object instanceof Character) ? string
					.lastIndexOf((Character) object) : -1;
		}
	...
	/**
	 * Returns a view of the specified {@code CharSequence} as a
	 * {@code List<Character>}, viewing {@code sequence} as a sequence of
	 * Unicode code units. The view does not support any modification
	 * operations, but reflects any changes to the underlying character
	 * sequence.
	 * 
	 * @param sequence
	 ...
	 * {@code Lists.reverse(Arrays.asList(1, 2, 3))} returns a list containing
	 * {@code 3,
	...
	private static class ReverseList<T> extends AbstractList<T> {
		...
		public ListIterator<T> listIterator(int index) {
			...
			final ListIterator<T> forwardIterator = forwardList
					.listIterator(start);
			return new ListIterator<T>() {
	...
	private static class RandomAccessListWrapper<E> extends
			AbstractListWrapper<E> implements RandomAccess {
